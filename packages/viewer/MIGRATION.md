...
### Behavior changes (per minor release)

- **`<Flipbook>` prints all-landscape documents on landscape paper** (in `4.2.0`) — When EVERY page of a document is wider than it is tall, printing injects `@page { size: landscape; margin: 0; }` for the duration of the job and removes it afterwards. Previously such a document printed onto portrait sheets, using about 43% of each page. Portrait, square and mixed-orientation documents emit no rule and are unaffected.

  Two consequences for an all-landscape document. A browser that honours a CSS page size also **removes the print dialog's orientation control** — confirmed in both Chrome and Firefox, where the row disappears entirely rather than being disabled. Paper size, scale, margins and pages-per-sheet remain available. And a consumer's own `@page { size: ... }` is overridden while the job runs. Opt out of both with `<Flipbook printOrientation="none">` (also added in `4.2.0`), which restores the previous behaviour exactly.

  Verified in all three engines. Chrome and Firefox honour the rule: the preview is landscape and the sheet count is unchanged. **Safari ignores it** — the preview stays portrait, exactly as before this release, and Safari's own Orientation control remains available for readers who want to switch manually. No engine changed the number of sheets.

- **`<Flipbook>` renders clickable link overlays automatically** (in `3.4.0`) — Any `source` that implements `getLinks()` (both the built-in `PdfjsSource` and the api-adapter's `PreRenderedPageSource`) now has its links rendered as clickable hit targets on top of each page. External links open in a new tab; internal links navigate via `actions.goToPage`. This is a behavior change for consumers already using `PreRenderedPageSource` — their pages will suddenly show link overlays where none existed before. Opt out with `<Flipbook showLinks={false}>` (also added in `3.4.0`). Consumers who don't implement `getLinks()` on their source see no change.

  ⚠️ **`PreRenderedPageSource` consumers MUST re-bake sidecar bundles with `@flipbookjs/api-adapter@1.6.0` (coordinated release) before pinning viewer `3.4.0`.** Bundles produced by earlier api-adapter versions contain broken link data — rotation-misaligned rects AND `destPage: 0` placeholders that make every internal link jump to page 1. If you must pin `3.4.0` before re-baking, set `<Flipbook showLinks={false}>` in the transition window to hide the broken overlays. Runtime `PdfjsSource` (i.e., `<Flipbook url={pdfUrl}>`) is unaffected — it generates link data at page load, not at bake time.
...
- **`PdfjsSourceOptions.{wasmUrl, standardFontDataUrl, cMapUrl, cMapPacked, iccUrl}`** (added in `3.3.0`) — Five new optional fields on `PdfjsSourceOptions` for overriding PDF.js's runtime asset URLs. When omitted, `PdfjsSource.init()` defaults each URL to jsDelivr — `https://cdn.jsdelivr.net/npm/pdfjs-dist@${pdfjs.version}/{wasm,standard_fonts,cmaps,iccs}/` — so PDFs using JPEG2000 (JPXDecode) images, JBIG2 images, ICC color-managed images, non-Latin scripts (CJK / Arabic / Hebrew), or standard-14 fonts without embedded glyphs render correctly out of the box (previously blank / mis-rendered). Consumers with strict CSP, air-gapped deployments, or privacy sensitivity override each URL individually — typical override is a self-hosted copy from their `node_modules/pdfjs-dist/{wasm,cmaps,standard_fonts,iccs}/` peer install. `cMapPacked` defaults to `true` (matches the packed CMap format jsDelivr / npm ships). Covered by `1.x` additive-evolution rules from `3.3.0` forward.
- **`FlipbookProps.pdfjsOptions?: PdfjsSourceOptions`** (added in `3.3.0`) — New optional prop on `<Flipbook>` that forwards options to the internally-constructed `PdfjsSource` when using the `url` prop path. Ignored when `source` is provided directly (consumers passing `source` already build their own `PdfjsSource` with any options). Primary use is overriding the runtime asset URLs above without switching from `<Flipbook url={...}>` to the more verbose `source={new PdfjsSource(url, {...})}` pattern. Consumers should memoize the object (`useMemo`) — a fresh inline literal each render re-creates the internal `PdfjsSource`.
- **`PdfjsSource.getLinks(index, signal?)`** (added in `3.4.0`) — Implementation of the optional `PageSource.getLinks?()` method for the runtime PDF.js source. Supported: **external URL links** (`<a target="_blank" rel="noopener noreferrer">` with an `aria-label`; `https:`, `http:`, `mailto:`, `tel:` schemes accepted by default; consumers extend via `PdfjsSourceOptions.additionalLinkSchemes`) and **intra-document links** (explicit-destination and named-destination — the latter resolved via `doc.getDestination`; internal-link clicks navigate via `actions.goToPage`). Deliberately dropped: JavaScript-action bundles (`actions`), Named viewer commands (`action` — any non-null value), FileAttachment (`attachment`), OCG state changes (`setOCGState`), form reset (`resetForm`), and any URL with a `javascript:`, `data:`, `vbscript:`, `file:` scheme (these cannot be re-enabled via `additionalLinkSchemes`). NOT distinguished from URI links: `Launch` actions and `GoToR` remote-PDF links whose targets surface as allowed-scheme URLs — pdfjs normalizes all three action types into a single `url` field before the annotation reaches us. Browser context makes the distinction moot (an `<a href={http://...}>` never launches an external application regardless of the source PDF's action type). Sanitized-capped at 1000 links per page. Covered by `1.x` additive-evolution rules from `3.4.0` forward.
- **`PageSource.getLinks?(index, signal?)` signature evolution** (added in `3.4.0`) — Optional `signal?: AbortSignal` param added. Existing implementations that ignore it continue to satisfy. Additive.
