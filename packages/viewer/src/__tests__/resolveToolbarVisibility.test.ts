...
describe('resolveToolbarVisibility — default rules', () => {
  it('showPrint defaults true when consumer prop is undefined', () => {
    const r = resolveToolbarVisibility({}, { canDownload: false, canFullScreen: false, isOverflowing: false, printError: null });
    expect(r.showPrint).toBe(true);
  });

  it('showDownload defaults to slice.canDownload', () => {
    expect(
      resolveToolbarVisibility({}, { canDownload: false, canFullScreen: false, isOverflowing: false, printError: null }).showDownload,
    ).toBe(false);
    expect(
      resolveToolbarVisibility({}, { canDownload: true, canFullScreen: false, isOverflowing: false, printError: null }).showDownload,
    ).toBe(true);
  });

  it('showFullScreen defaults to slice.canFullScreen', () => {
    expect(
      resolveToolbarVisibility({}, { canDownload: false, canFullScreen: false, isOverflowing: false, printError: null }).showFullScreen,
    ).toBe(false);
    expect(
      resolveToolbarVisibility({}, { canDownload: false, canFullScreen: true, isOverflowing: false, printError: null }).showFullScreen,
    ).toBe(true);
  });
...
describe('resolveToolbarVisibility — consumer overrides', () => {
  ...
  it('consumer false forces hidden even when default would be true', () => {
    const r = resolveToolbarVisibility(
      ...
      { canDownload: true, canFullScreen: true, isOverflowing: false, printError: null },
    );
